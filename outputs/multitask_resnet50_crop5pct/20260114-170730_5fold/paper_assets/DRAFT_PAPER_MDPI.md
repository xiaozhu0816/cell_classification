...
> This version is updated to use the **final crop-5\% experiment** located at:
> `outputs/multitask_resnet50_crop5pct/20260114-170730_5fold`
...
To mitigate possible information leakage due to the approximately \~5\% field-of-view overlap between adjacent acquisition positions reported by GMU, we applied a simple preprocessing step in the final experiment: **we center-cropped each frame to remove the outer 5\% border (retaining the central 90\%\,$\times$\,90\% region) prior to resizing**. This reduces the chance that near-duplicate border content appears across train/test folds.

**Results:** Across five folds, the model achieved classification accuracy **0.9925 ± 0.0010**, F1-score **0.9924 ± 0.0010**, and AUC **0.9998 ± 0.0001**. We report time prediction error in **hours** (MAE **1.176 ± 0.097 h**; RMSE **1.491 ± 0.102 h**), corresponding to ~2.35 frames at a 30 min imaging interval (reported as a secondary, intuitive scale). Temporal profiling showed lower performance in early-stage windows (e.g., window center 3 h: accuracy 0.944; recall 0.883; AUC 0.991) with rapid improvement thereafter (from window center 9–12 h onward, accuracy/F1/AUC ≈ 1.0).
...
## Data and outputs in this repo (final experiment)

- Results directory: `outputs/multitask_resnet50_crop5pct/20260114-170730_5fold`
- Paper assets directory: `outputs/multitask_resnet50_crop5pct/20260114-170730_5fold/paper_assets`
- Tables:
	- `paper_table_overall_metrics.csv`
	- `paper_table_temporal_metrics.csv`
- Figures already generated:
	- `figure_showme_mock_vs_infected_4rows.png` (NEW: real microscopy montage; 2 stacks per condition)
	- `figure_showme_mock_vs_infected.png` (alt: 1 stack per condition)
...
Across five CV folds, we observed highly consistent classification performance (accuracy **0.9925 ± 0.0010**, F1 **0.9924 ± 0.0010**, AUC **0.9998 ± 0.0001**) and hour-level time prediction accuracy (MAE **1.176 ± 0.097 h**, RMSE **1.491 ± 0.102 h**) (Table 1). The regression scatter plot (Figure 3A) demonstrates close alignment between predicted and ground-truth times for both infected and uninfected samples.
...
- **Potential correlation structure / overlap:** patch overlap and within-position correlations can inflate effective sample size. While position-level splitting mitigates direct leakage, acquisition-level field-of-view overlap may still introduce near-duplicate border content. The final experiment therefore applies a simple border-removal preprocessing (crop 5\% per side) to further mitigate this risk.
...
### 4.2 Anti-overlap preprocessing (final experiment)

Because GMU reported an approximate ~5\% field-of-view overlap between adjacent acquisition positions, the final experiment applies a robust preprocessing step to reduce near-duplicate border content across folds: **we remove the outer 5\% border of each frame by center cropping (retain central 90\%×90\%), then resize to the model input resolution**. This preprocessing is applied consistently to train/validation/test.
...
**Reproducibility:** All experiments were run with a fixed random seed (seed = 42) and a fixed 5-fold split. Hyperparameters were defined in a YAML configuration file (e.g., `configs/multitask_example_crop5pct.yaml`) and the cross-validation training script (`train_multitask_cv.py`) wrote all fold outputs (checkpoints, predictions, and per-fold metrics) to a timestamped results directory.
